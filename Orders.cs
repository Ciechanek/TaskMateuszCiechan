...
namespace TaskMateuszCiechan
{
    public class Orders
    {
        ...
        private double expectedOrderValue;
        private double priceOfElement;
        private double orderValue;
        private string priceOfElementStr;
        ...
        public void CheckIfThereIsCorrectPrice()
        {
            loginTest.webDriver = new ChromeDriver();
            loginTest.EnterPageAndLogIn("standard_user", "secret_sauce");

            //first item
            loginTest.webElement = loginTest.webDriver.FindElement(By.Id("add-to-cart-sauce-labs-backpack")); //here i had a problem - this is very bad way in my case
            loginTest.webElement.Click();

            priceOfElementStr = loginTest.webDriver.FindElement(By.XPath("(//div[@class='inventory_item_price'])[1]")).Text;
            priceOfElementStr = priceOfElementStr.Trim('$');
            priceOfElementStr = priceOfElementStr.Replace('.', ',');

            priceOfElement = Convert.ToDouble(priceOfElementStr);
            expectedOrderValue += priceOfElement;

            //second item
            loginTest.webElement = loginTest.webDriver.FindElement(By.Id("add-to-cart-sauce-labs-bike-light")); //here i had a problem - this is very bad way in my case
            loginTest.webElement.Click();

            priceOfElementStr = loginTest.webDriver.FindElement(By.XPath("(//div[@class='inventory_item_price'])[2]")).Text;
            priceOfElementStr = priceOfElementStr.Trim('$');
            priceOfElementStr = priceOfElementStr.Replace('.', ',');
            ...
            orderValueStr = loginTest.webDriver.FindElement(By.XPath("(//div[@class='summary_subtotal_label'])")).Text;
            orderValueStr = orderValueStr.Substring(orderValueStr.Length - 5);
            orderValueStr = orderValueStr.Replace('.', ',');

            orderValue = Convert.ToDouble(orderValueStr);

            try
            {
                Assert.AreEqual(expectedOrderValue, orderValue);
                string text = "Checking correct price test passed succesfully, total price is correct\n\n";
                File.AppendAllText("../../../TestReportMateuszCiechan.txt", text);
            }
            catch
            {
                string text = "Checking correct price test not passed\n\n";
                File.AppendAllText("../../../TestReportMateuszCiechan.txt", text);
            }
            loginTest.webDriver.Close();
        }
